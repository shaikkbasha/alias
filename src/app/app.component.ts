...
declare let ga: any; // For Dynamic analytic creation please don't remove.
@Component({
  selector: 'app-root',
...
export class AppComponent implements OnInit {
  ...
  constructor(public oktaAuth: OktaAuthService,
    ...
    private angulartics2: Angulartics2, private googleAnalytics: Angulartics2GoogleAnalytics, private router: Router,
    private activateRoute: ActivatedRoute) {
    /* Disabled Google Analytics in Localhost */
    if (!environment.baseUrl.includes('localhost')) {
      ga('create', environment.GOOGLE_ANALYTICS_ID, 'none');
      angulartics2.setUsername.next(environment.GOOGLE_ANALYTICS_ID);
      router.events.subscribe((urlObj) => {
        if (urlObj instanceof NavigationEnd) {
          const excludedUrls = [
            '/home',
            '/login',
            '/implicit/callback'
          ];
          let url = urlObj.url.split('?')[0]; /* Removed Query Params */
          /* Removed Hash Params */
          if (url.includes('#')) {
            url = url.split('#')[0];
          }
          if (!excludedUrls.includes(url)) { /* Filter Exclued Urls */
            /* Removed Dynamic Ids from Router Params Like Airline, Flight Id, Tail Id etc */
            if (activateRoute.firstChild && activateRoute.firstChild.snapshot && activateRoute.firstChild.snapshot.firstChild) {
              const params = activateRoute.firstChild.snapshot.firstChild.params;
              if (Object.entries(params).length > 0) {
                Object.entries(params).forEach(param => {
                  url = url.replace(`/${params[param[0]]}`, ''); /* Replace Dyanmic Router Params Here empty string */
                });
              }
            }
            const isUpperCase = /[A-Z]/.test(url);
            if (!isUpperCase) {
              /* Here start tracking */
