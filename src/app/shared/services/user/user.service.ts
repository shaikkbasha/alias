...
export class UserService {
  ...
  public checkUserRole = this.userInformation.asObservable();
  ...
  getUserInfo(userClaims) {
    return this.userInformation.next(userClaims); // Check User Role
  }
  ...
  forceRedirectToBack(url, queryParams, param) {
    if (url.includes(param) && !url.includes('fromDate') && !url.includes('toDate')) {
      return {
        isIncrement: true
      };
    } else if (url.includes(param) && url.includes('fromDate') && url.includes('toDate')) {
      let res = {};
      queryParams.subscribe(params => {
        if (params && Object.keys(params).length) {
          res = {
            fromdates: params.fromDate.replace(/(^|\D)(\d)(?!\d)/g, '$10$2'),
            todates: params.toDate.replace(/(^|\D)(\d)(?!\d)/g, '$10$2'),
            isIncrement: false
          };
        }
      });
  ...
  dateValidation(fromdates, todates) {
    if (new Date(fromdates) > new Date(todates)) {
      const date = new Date();
