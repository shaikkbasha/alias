...
export class RemovalsService {
  ...
  getShowDetails(id: any): Observable<any[]> {
    return this.http.get<any[]>(this.ADMIN_API + `/${id}`)
      .pipe(
        catchError(this.handleError('getShowDetails', []))
      );
  }
  getActionDetails(id: any): Observable<any[]> {
    return this.http.get<any[]>(this.API + `/api/v1/products/lru-part-numbers/${id}/repair-actions`)
      .pipe(
        catchError(this.handleError('getShowDetails', []))
      );
  }
