...
describe('TunerService', () => {
  ...
  xit('should get tuner board details', () => {
    ...
    const data = `${tunerService.ADMIN_API}${airlineIcao}/tv-performance/flights/${flightId}/boards/`;
    const tunerReq = httpMock.expectOne(data + `${boardObj.boardId}/tuners/${boardObj.tunerId}`);
  ...
  it('should get  export TunnerData', () => {
    const filters = {
      tunerParam: 'agc',
      boardParam: 2
    };
    ...
    const flightId = '5c011a1e53b7fd001fec504a';
    tunerService.ADMIN_API = `${environment.API}/api/v1/airline/`;
    tunerService.exportTunnerData(airlineIcao, flightId, filters).subscribe(res => {
      console.log(res);
    });
  });
  it('should get  export Board TunnerData', () => {
    const filters = {
      boardId: 1,
      tunerId: 1
    };
    ...
    const flightId = '5c011a1e53b7fd001fec504a';
    tunerService.ADMIN_API = `${environment.API}/api/v1/airline/`;
    tunerService.exportBoardTunnerData(airlineIcao, flightId, filters).subscribe(res => {
      console.log(res);
    });
  });
});
