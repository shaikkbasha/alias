...
describe('AntennaService', () => {
  ...
  it('should get aiports', () => {
    const airports = `ICN,JFK`;
    antennaService.AIRPORT_API = `${environment.API}/api/v1/airports`;
    antennaService.getAllAirportData(airports).subscribe(res => {

    });
    const antennaReq = httpMock.expectOne(`${antennaService.AIRPORT_API}?iata=${airports}`);
    expect(antennaReq.request.method).toEqual('GET');
    antennaReq.flush(null);
    httpMock.verify();
  });
