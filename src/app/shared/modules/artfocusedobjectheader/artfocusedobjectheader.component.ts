...
export class ArtfocusedobjectheaderComponent implements OnInit {
  ...
  focussednavigationSubscription;
  constructor(private route: ActivatedRoute, private router: Router) { }

  ngOnInit() {
    this.focussednavigationSubscription = this.router.events.subscribe((event: any) => {
  ...
  isFocussedActive(instruction: any[]): boolean {
  ...
  isFocussedDropdownItemActive(instructions: any[][]): boolean {
    // https://stackoverflow.com/questions/39271654/routerlinkactive-for-routerlink-with-parameters-dynamic
