...
describe('TunerComponent', () => {
  ...
  const flightDataExportTuner = {
    'id': '5ca3679b814514000d1a0bc8',
    'tailNumber': 'JB1TEST4',
    'flightNumber': '326',
    'departureTime': '2019-04-02T06:43:48Z',
    'arrivalTime': '2019-04-02T09:43:48Z',
  ...
  const mockTunerService = {
    ...
    exportTunnerData(): Observable<any> {
      return Observable.of(new Blob([]));
    },
    exportBoardTunnerData(): Observable<any> {
      return Observable.of(new Blob([]));
    }
  };
  ...
  it('should trigger export tuner data', () => {
    component.flightDetails = flightDataExportTuner;
    component.tunerView = 'parameter';
    if (component.tunerView === 'parameter') {
      const eventObj = {
        moduleName: 'tuner',
        eventName: 'export',
        tunerParam: 'agc',
      ...
      const spy = spyOn(tunerService, 'exportTunnerData').and.callThrough();
      component.actionExportTunerEvent(eventObj);
      expect(spy).toHaveBeenCalled();
    }
  });

  it('should create tuner file name', () => {
    component.flightDetails = flightDataExportTuner;
    const fileName = component.getExportFileName();
    const fileNamewithParameter = fileName.slice(0, fileName.lastIndexOf('_'));
    const fileNamewithParameter2 = fileNamewithParameter.concat('_agc');
    expect(fileNamewithParameter2).toBe('Tuner_JBU_JB1TEST4_326_20190402064348_Board_1_agc');
  });

  it('should trigger export board tuner data', () => {
    component.flightDetails = flightDataExportTuner;
    component.tunerView = 'boardTuner';
    if (component.tunerView === 'boardTuner') {
      const eventObj = {
        moduleName: 'tuner',
        eventName: 'export',
        boardId: '1',
      ...
      const spy = spyOn(tunerService, 'exportBoardTunnerData').and.callThrough();
      component.actionExportTunerEvent(eventObj);
      expect(spy).toHaveBeenCalled();
    }
  });

  it('should create board tuner file name', () => {
    component.flightDetails = flightDataExportTuner;
    const fileName = component.getExportFileNameBoardtuner();
    const fileNamewithParameter = fileName.slice(0, fileName.lastIndexOf('_'));
    const fileNamewithParameter2 = fileNamewithParameter.concat('_1');
    expect(fileNamewithParameter2).toBe('Tuner_JBU_JB1TEST4_326_20190402064348_Board_1_Tuner_1');
  });
