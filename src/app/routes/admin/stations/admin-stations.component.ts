...
export class AdminStationsComponent implements OnInit {
  ...
  */
  ...
  @ViewChild('msSort') msSort: MatSort;
  ...
  getMaintenanceStationsList() {
    ...
    this.maintenanceStationService.getMaintenanceStationlist().subscribe(list => {
      ...
      if (data.error) {
        ...
        this.msSortData({active: 'fullName', direction: 'asc'}, 'msSort');
      }
      this.dataSource.paginator = this.paginator.toArray()[0];
      this.dataSource.sort = this.msSort;
  ...
  */
  getRepairStationList() {
    ...
    this.repairStationsService.getRepairStationslist().subscribe(list => {
      ...
      if (data.error) {
        ...
        this.msSortData({active: 'fullName', direction: 'asc'}, 'rsSort');
      }
  ...
  updateMaintenanceStation(errorHandlingId) {
    ...
    this.maintenanceStationService.updateMaintenanceStations(this.maintenanceStationsForm.value).subscribe(response => {
      ...
      if (data && !data.error) {
        ...
        this.dataSource.sort = this.msSort;
        this.msSortData({active: 'fullName', direction: this.msSort.direction}, 'msSort');
      } else if (data.error) {
    ...
    if (!this.repairStationsForm.value.id) {
      ...
      this.repairStationsService.createRepairStations(this.repairStationsForm.value).subscribe(response => {
        ...
        if (data && !data.error) {
          ...
          this.repairStationsDataSource.sort = this.rsSort;
          this.msSortData({active: 'fullName', direction: this.rsSort.direction}, 'rsSort');
        } else if (data.error) {
  ...
  sortData(sort, data) {
    return data.sort((a, b) => {
      ...
      switch (sort.active) {
        ...
        case 'locationName': return this.compare(a.locationName, b.locationName, isAsc);
  ...
  msSortData(sort, tabName) {
    let data = [];
    if (tabName === 'msSort') {
      data = this.dataSource.data;
      this.dataSource = new MatTableDataSource<MaintenanceStation>(data);
    } else if (tabName === 'rsSort') {
      data = this.repairStationsDataSource.data;
      this.repairStationsDataSource = new MatTableDataSource<RepairStation>(data);
    }
    ...
    if (tabName === 'msSort') {
      this.dataSource = new MatTableDataSource<MaintenanceStation>(stationsDataSource);
      this.dataSource.paginator = this.paginator.toArray()[0];
      this.dataSource.sort = this.msSort;
    } else if (tabName === 'rsSort') {
      this.repairStationsDataSource = new MatTableDataSource<RepairStation>(stationsDataSource);
      this.repairStationsDataSource.paginator = this.paginator.toArray()[1];
      this.repairStationsDataSource.sort = this.rsSort;
    }
