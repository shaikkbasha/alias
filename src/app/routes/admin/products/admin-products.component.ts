...
export class AdminProductsComponent implements OnInit {
  enablePartNumberTable = false;
  partnumberdisplayColumns = ['partNumber', 'alias', 'createdAt', 'updatedAt'];
  partnumberDisplayHeader = ['NAME', 'ALIAS', 'CREATED AT', 'UPDATED AT'];
  nodes: any = [{ name: '' }];
  dataSource: any = [];
  getSelectedTabName = '';
  deleteObj: any = {
  ...
  partNumberDisplayedColumns: string[] = ['select', 'partNumber', 'alias', 'createdAt', 'updatedAt'];
  ...
  ngOnInit() {
    ...
    this.repairAction = productsObj;
    this.repairAction.headText = 'Create Repair Action';
    this.repairAction.form.obj.saveButtonLabel = 'Save repair action';

    this.reasonOfRemoval = productsObj;
    this.reasonOfRemoval.headText = 'Create reasons of removal';
    this.reasonOfRemoval.form.obj.saveButtonLabel = 'Save reason of removal';
    ...
    this.productsService.getTreeList().subscribe((data) => {
      ...
      if (data && !data['error']) {
        ...
        for (let i = 0; i < list.length; i++) {
          for (let j = 0; j < list[i].lruNames.length; j++) {
            for (let pn = 0; pn < list[i].lruNames[j].partNumbers.length; pn++) {
              const partno = list[i].lruNames[j].partNumbers[pn];
              if (!!partno.alias) {
                list[i].lruNames[j].partNumbers[pn].name = partno.partNumber + '(' + partno.alias + ')';
              }
            }
          }
        }
  ...
  getNode(args) {
    ...
    const getLists = [];
    // getLists.push(args);
    this.enablePartNumberTable = false;
    this.productsObj.getSelectedNode = args;
    this.selection.clear();
    this.productsObj.form.name = '';
    this.productsObj.form.saveButtonLabel = '';
    ...
    if (Object.keys(args).indexOf('lruNames') !== -1) {
      ...
      this.productsObj.config.createLabel = 'Create LRU Name';
      this.productsObj.displayHeader[0] = 'LRU NAME';
      this.productsObj.config.id = 'btn-admin-product-createLruName';
      this.productsObj.config.moduleName = 'createLRUName';

    } else if (Object.keys(args).indexOf('partNumbers') !== -1) {
      this.enablePartNumberTable = true;
      this.productsObj.form.name = 'Create Part Number';
      this.productsObj.form.saveButtonLabel = 'Save Part Number';
      this.productsObj.config.createLabel = 'Create Part Number';
      this.productsObj.config.id = 'btn-admin-product-createPartNumber';
      this.productsObj.config.moduleName = 'createPartNumber';
      ...
      this.productsObj.displayHeader[0] = 'LRU PART NUMBER NAME';
      this.productsObj.config.createLabel = 'Create LRU Part Number';
      this.productsObj.config.id = 'btn-admin-product-createLRUPartNumber';
      this.productsObj.config.moduleName = 'createLRUPartNumber';
        ...
        lruType.lruNames.filter((lruPartNumbers, lruPartNumbersIndex) => { // GET PART NO
          ...
          if (lruPartNumbers.partNumbers && lruPartNumbers.partNumbers.length) {
            lruPartNumbers.partNumbers.filter((partNumbers, partNumbersIndex) => { // GET LRU PART NUMBERS
              ...
              data.lruTypes[lruTypeIndex].lruNames[lruPartNumbersIndex].partNumbers[partNumbersIndex].name = lruPId.partNumber+'sal';
  ...
  saveProducts(apiMethod, formData, isPartNumber?) {
      ...
      if (data && !data.error) {
        ...
        if (this.productsObj.form.name.indexOf('Create') !== -1) {
          ...
          if (!data.name && data.partNumber) {
            data.name = isPartNumber ? data.partNumber + '(' + data.alias + ')' : data.partNumber;
        ...
        this.msSortData({ active: 'name', direction: this.pSort.direction });
  ...
  openDialogModal(evt) {
    if (evt.moduleName === 'createLRUType' && evt.eventName === 'create') {
      ...
      if (evt.moduleName === 'createLRUName') {
        const formData = JSON.parse(JSON.stringify(this.productsObj.lruConfigData));
        formData[0].value='';
        // let aliasIndex = formData.findIndex( (form) => form.key =='alias');
        // console.log('aliasIndex-->>',aliasIndex)
        // formData.push({
        //   key: 'alias',
        //   label: 'Alias',
        //   id: 'alias',
        ...
        //   order: 1
        // });
        this.productsObj.lruTypeForm = this.dynamicFormService.getFrom(formData);
    ...
    if (this.productsObj.form.name.indexOf('Create') !== -1) {
      ...
      if (this.productsObj.form.name === 'Create LRU Type') {
        ...
        const input = {
          ...
          alias: data.formData.alias
        };
        apiMethod = this.partNumberService.createPartNumber(input);
        this.saveProducts(apiMethod, formData, true);
      ...
      if (this.productsObj.form.name === 'Update LRU Type') {
        ...
        this.saveProducts(this.partNumberService.updatePartNumber(input), formData, true);
    ...
    if (this.productsObj.getIndexOfNodes.length === 1) {
      ...
      this.nodes[this.productsObj.getIndexOfNodes[0]]['children'].filter((val, index) => {
        if (parseFloat(val.id.split('-')[1]) === parseFloat(data.id.split('-')[1])) {
          const obj = ['createdAt', 'id', 'lruTypeId', 'name', 'updatedAt'];
          for (let i = 0; i < obj.length; i++) {
            this.nodes[this.productsObj.getIndexOfNodes[0]]['children'][index][obj[i]] = data[obj[i]];
          }
          isIndex = true;
        }
      });
      ...
      this.ascSorting(this.nodes[this.productsObj.getIndexOfNodes[0]]['children']);
        ...
        if (parseFloat(val.id.split('-')[1]) === parseFloat(data.id.split('-')[1])) {
          ...
          if (data.children) {
            for (let i = 0; i < data.children.length; i++) {
              data.children[i].name = data.name + '-' + data.children[i].name.split('-')[1];
            }
          }
      ...
      this.ascSorting(this.nodes[pNo[0]]['children'][pNo[1]]['children']);
      ...
      this.ascSorting(this.nodes[pNo[0]]['children'][pNo[1]]['children'][pNo[2]]['children']);
      ...
      this.ascSorting(this.nodes[pNo[0]]['children'][pNo[1]]['children'][pNo[2]]['children'][pNo[3]]['children']);
      ...
      if (getIndex !== null) {
        ...
        for (let i = 0; i < obj.length; i++) {
          this.nodes[getIndex][obj[i]] = data[obj[i]];
        }
      } else {
        this.nodes.push(data);
      }
      this.ascSorting(this.nodes);
    }
  }
  deleteTreeNodes() {
  ...
  getTabName(event) {
    ...
    if (event.tab.textLabel === 'Reasons of Removal') {
      ...
      this.productsObj.config.id = 'btn-admin-product-reasonofremovaL';
      const apiMethod = {
      ...
      this.productsObj.config.id = 'btn-admin-product-details';
      ...
      this.productsObj.config.id = 'btn-admin-product-repairactions';
      const apiMethod = {
