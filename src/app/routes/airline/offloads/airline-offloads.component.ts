...
export class AirlineOffloadsComponent implements OnInit, OnDestroy {
  ...
  data: any = [
    ...
    { title: 'PROCESSED', filterValue: 'processed', filterKey: 'status', filterRule: 'no'},
    { title: 'REJECTED', filterValue: 'rejected', filterKey: 'status', filterRule: 'yes'}
  ];
  widgetFilterObj: any = {
    allOffloads: [],
    processed: [],
  ...
  fetchOffLoads() {
    ...
    this.offloadService.getOffloadList(this.id, this.fromDate, this.toDate).subscribe(list => {
      ...
      data.forEach(function (ele) {
        ...
        if (ele['tailNumber'] === null || ele['tailNumber'] === '') {
          ele['tailNumber'] = ele['tailNumberInFile'];
        }
        offloadData.push(ele);
      });

      if (data.error) {
        this.dataSource = new MatTableDataSource<OffloadModel>([]);
      } else {
        this.dataSource = new MatTableDataSource<OffloadModel>(offloadData);
      }
