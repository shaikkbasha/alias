import { TemplateRef, Component, OnInit, ViewChild, OnDestroy } from '@angular/core';
...
export class AirlineFlightsComponent implements OnInit, OnDestroy {
  enableSearchToolBar = true;
  displayedColumns: string[] = ['tailNumber', 'flightNumber', 'arrivalAirport', 'departureAirport',
    'departureDate', 'arrivalDate', 'isDarkFlight', 'biteOffloadReceived', 'action'];
  modalRef: any;
  ...
  data: any = [
    { title: 'ALL FLIGHTS', filterValue: 'all', filterKey: 'biteOffloadReceived' },
    { title: 'RECEIVED OFFLOADS', filterValue: 'yes', filterKey: 'biteOffloadReceived', filterRule: 'no' },
    { title: 'MISSING OFFLOADS', filterValue: 'no', filterKey: 'biteOffloadReceived', filterRule: 'yes' }
  ];
  widgetFilterObj: any = {
    allFlights: [],
    biteOffloadReceived: [],
  ...
  fetchFlightLists() {
    ...
    this.flightService.getFlightList(this.id, this.fromDate, this.toDate, '', true).subscribe(list => {
      const data: any = list;
      if (data.error) {
  ...
  updateFormDetails(details) {
    this.flightObj.getFlightDetails = this.selectedRow;
    this.flightObj.formObj.airline = this.id.toUpperCase();
    this.flightObj.formObj.tail = this.selectedRow.tailNumber;
  ...
  updateFlightDetails() {
    ...
    if (this.flightObj.formObj.isDarkFlight.toUpperCase() === 'YES' && !this.flightObj.formObj.openDate) {
      ...
      this.flightService.updateFlightDetails(this.id, this.selectedRow.id, updateObj).subscribe(response => {
        this.flightObj.isFormSubmitted = false;
        const data: any = response;
        if (data && !data.error && (data.status === 200 || !data.status)) {
          const msg = this.selectedRow.flightNumber + ' flight was successfully updated';
          this.toaster.success(msg, 'Update Flight');
          this.dataSource.data.splice(this.selectedRow, 1);
          response['tailNumber'] = this.flightObj.formObj.tail;
          response['biteOffloadReceived'] = this.flightObj.getFlightDetails.biteOffloadReceived;
  ...
  editFlight(element) {
    ...
    this.flightService.getFlightDetails(this.id, this.selectedRow.id).subscribe(response => {
